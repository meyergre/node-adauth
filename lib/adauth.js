...
ADAuth.prototype.authenticate = function (username, password, callback) {
  ...
  //
  // EDIT meyergre : replaced bind config to fit my needs in a particular project
  // previously :
  // self._userClient.bind(username, password, function (err) {
  //
  self._userClient.bind(this.opts.bindDn, this.opts.bindCredentials, function (err) {
    if (err) {
    ...
    self._findUser(username, opts, function (err, user) {
      if (err)
        return callback(err);
      if (!user)
        return callback(format('cannot find user object for: "%s"', username));
      // 3. authenticate user by its DN
      self._userClient.bind(user.dn, password, function (err) {
        ...
        self._getGroups(user, opts, function (err, user) {
          if (err) {
            ...
            return callback(format('cannot authenticate user: "%s"', username));
          }
          if (self.opts.cache) {
